...
namespace ae2f {
	namespace Bmp {
		namespace Dot {
			...
			struct rRGB {
				uint32_t inst;
				constexpr rRGB(uint32_t a) : inst(a) {}
				constexpr rRGB(uint8_t r, uint8_t g, uint8_t b) 
					: inst(ae2f_Bmp_Dot_RGB_Make(r, g, b)) {}

				constexpr uint8_t R() const {
					return ae2f_Bmp_Dot_RGBA_GetR(this->inst);
				}
				constexpr const rRGB R(uint8_t val) const {
				...
				constexpr const rRGB G(uint8_t val) const {
				...
				constexpr const rRGB B(uint8_t val) const {
					return ae2f_Bmp_Dot_RGBA_SetB(this->inst, val);
				}
			};
			struct rRGBA : public rRGB {
				constexpr rRGBA(uint32_t a) : rRGB(a) {}
				constexpr rRGBA(uint8_t r, uint8_t g, uint8_t b, uint8_t a = 255)
					: rRGB(ae2f_Bmp_Dot_RGBA_Make(r, g, b, a)) {}
				constexpr rRGBA(const rRGB rgb, uint8_t a = 255) 
					: rRGB(ae2f_Bmp_Dot_RGBA_FromRGB(rgb.inst, a)) {}
				constexpr uint8_t A() const {
					return ae2f_Bmp_Dot_RGBA_GetA(this->inst);
				}
				constexpr const rRGBA A(uint8_t a) const {
					return ae2f_Bmp_Dot_RGBA_SetA(this->inst, a);
				}
			};

			template<eBlendPostFix::_ = eBlendPostFix::F>
			const rRGB Blend(const rRGB background, const rRGBA colour_to_blend);

			template<eBlendPostFix::_ = eBlendPostFix::F>
			const rRGB Blend(const rRGBA __colour_A, const rRGBA __colour_B);

			template<>
			const rRGB Blend<eBlendPostFix::F>(const rRGB background, const rRGBA colour_to_blend) {
				return ae2f_Bmp_Dot_rRGBA_BlendRGB(background.inst, colour_to_blend.inst, f);
			}

			template<>
			const rRGB Blend<eBlendPostFix::F>(const rRGBA __colour_A, const rRGBA __colour_B) {
				return ae2f_Bmp_Dot_rRGBA_BlendRGBA(__colour_A.inst, __colour_B.inst, f);
			}

			template<>
			const rRGB Blend<eBlendPostFix::D>(const rRGB background, const rRGBA colour_to_blend) {
				return ae2f_Bmp_Dot_rRGBA_BlendRGB(background.inst, colour_to_blend.inst, );
			}

			template<>
			const rRGB Blend<eBlendPostFix::D>(const rRGBA __colour_A, const rRGBA __colour_B) {
				return ae2f_Bmp_Dot_rRGBA_BlendRGBA(__colour_A.inst, __colour_B.inst, );
			}

			template<>
			const rRGB Blend<eBlendPostFix::L>(const rRGB background, const rRGBA colour_to_blend) {
				return ae2f_Bmp_Dot_rRGBA_BlendRGB(background.inst, colour_to_blend.inst, l);
			}

			template<>
			const rRGB Blend<eBlendPostFix::L>(const rRGBA __colour_A, const rRGBA __colour_B) {
